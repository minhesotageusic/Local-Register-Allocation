	...
	while (head_ref != NULL) {
		...
		if (data != NULL) {
			...
			if (assignedRegister == NULL)
				...
				sprintf(str + stroffset, "%s, length: %d]\t|",
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		...
		if (read > 1 && !isOnlyWhiteSpace(copy, read) && line[0] != '/'
			...
			while (ptr != NULL) {
				...
				if (ptr[0] == 'r') {
					ret->highestNodeNumber = maxNodeNumber(ptr, ret->highestNodeNumber, strlen(ptr));
...
void printChanges_Bottom(FILE* fp, TopDownMetaNode * meta, char * delim, Node ** registerList_head,
		Node ** bottomUp_head, int feasibleCount) {
...
//file helper
	...
	//create physical set
	CreateFeasibleSet(&physical_reg_head_node, feasibleCount, meta->highestNodeNumber);
